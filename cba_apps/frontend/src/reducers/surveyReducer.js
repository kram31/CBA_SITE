  ...
  switch (action.type) {
    case GET_ALL_DATA:
      ...
      return {
        ...state,
        surveys: surveys,
        bottombox: surveys.filter(item => item.bottombox == 1 && !item.rca),
        pieChartCompletedSurveysCount: [
        ...
        yearSelection: [
          ...new Set(
            surveys.map(survey => new Date(survey.date_issued).getFullYear())
          )
        ],
        chart_data: get_data(
          surveys
    ...
    case DELETE_SURVEY:
      return {
        ...
        rcas: state.rcas.filter(item => action.payload != item.surveyed_ticket),
        isFetching: false
      };
    case ADD_SURVEY:
      return {
    ...
    case DELETE_DSAT_CODE1:
      return {
        ...state,
        dsat_code1: state.dsat_code1.filter(item => action.payload != item.id),
        isFetching: false
      };
    ...
    case ADD_RCA:
      return {
        ...
        bottombox: state.bottombox.filter(
          item => action.payload.surveyed_ticket != item.reference_number
        ),
    ...
    case UPDATE_SURVEY:
      return {
        ...
        surveys: state.surveys.map(survey => {
          if (survey.reference_number == action.payload.reference_number) {
            return action.payload;
          } else {
            return survey;
          }
    ...
    case UPDATE_AGENT:
      return {
        ...
        agents: state.agents.map(agent => {
          if (agent.operator_lan_id == action.payload.operator_lan_id) {
            return action.payload;
          } else {
            return agent;
          }
