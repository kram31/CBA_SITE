...
class DatatablePage extends Component {
  ...
  handleToggle = data => {
    data.completed == true ? this.props.getRca(data) : this.props.removeRca();
  ...
  render() {
        ...
        case "date_issued":
          return Object.assign({
            ...
            ["Filter"]: ({ filter, onChange }) => (
              <DateRangePicker onApply={(event, picker) => onChange(picker)}>
            ...
            ["filterMethod"]: (filter, row) => {
              ...
              filter ? console.log(filter) : console.log("no filter");

              let format = "YYYY-MM-DD";
              let date_q = row["date_issued"];
              let startD = moment(filter.value.startDate).format(format);
              let endD = moment(filter.value.endDate).format(format);

              if (moment(date_q).isBetween(startD, endD, null, [])) {
                return true;
              }
            }
          });
        ...
        <Fragment>
          <div className="btn-group">
            ...
            <Button
              ...
              size="sm"
              onClick={() =>
                this.handleDelete(cellprops.original.reference_number)
              }
            >
              Delete
            </Button>
    ...
    const surveysTableExcelSheet = (
      <ExcelSheet
        data={
          this.state.sortedData[0] ? this.state.sortedData : this.props.surveys
        }
        name="Surveys"
      >
      ...
      <Fragment>
          ...
          <Col md={6} style={{ display: "inline" }}>
            <Fragment>
              <ExcelFile
                filename="Bottombox"
                element={
                  <Button className="mr-2 btn-bb" onClick={this.handleClick}>
                    Download
                  </Button>
                }
              >
                ...
                {this.props.agents.length != 0 ? (
                  <ExcelSheet data={this.props.agents} name="Agents">
                    {this.props.agent_headers.map(item =>
                      item != "surveys" ? (
                        <ExcelColumn key={item} label={item} value={item} />
                      ) : (
                        <ExcelColumn key={item} label={item} value="test" />
                      )
                    )}
                  </ExcelSheet>
                ) : (
            ...
            <Button className="mr-2 btn-bb" onClick={this.filterToggle}>
              Filter
            </Button>
            <Button className="mr-2 btn-bb" onClick={this.resetFields}>
              Reset
            </Button>
          </Col>
        </Row>
        ...
        <ReactTable
          ...
          onFilteredChange={filtered => {
            this.setState({
              filtered,
              sortedData: this.reactTable.getResolvedState().sortedData
            });
          }}
